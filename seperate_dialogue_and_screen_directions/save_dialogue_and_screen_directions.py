...
DESTINATION_PATH = 'script_files/dialogue_and_screen_directions'
...
for filename in os.listdir(SCRIPTS_WITHOUT_TAGS_PATH):
    ...
    for whitespace, lines in filtered_whitespace_length_lines_pairs:
        if whitespace < average_whitespace:
            stage_directions.extend(lines)
        elif whitespace > average_whitespace:
            dialogue.extend(lines)
        else:
            # Already tested, 'whitespace' never equals 'average_whitespace' for this dataset
            continue

    with open(os.path.join(DESTINATION_PATH, filename), 'w') as destination_file:
        destination_file.writelines(map(lambda line: line + os.linesep, dialogue))
        destination_file.write(os.linesep * 100)
        destination_file.writelines(map(lambda line: line + os.linesep, stage_directions))
